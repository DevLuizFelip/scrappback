...
// --- Cache e Fila de Processamento ---
const cache = new Map();
const CACHE_DURATION_MS = 15 * 60 * 1000;
let isScraping = false; // Variável de controlo para a fila
...
// --- Endpoints da API ---
app.get('/api/sources', async (req, res) => { await db.read(); res.json(db.data.sources); });
app.delete('/api/sources/:id', async (req, res) => {
    const { id } = req.params;
    await db.read();
    const sourceToRemove = db.data.sources.find(s => s.id === id);
    if (!sourceToRemove) return res.status(404).json({ message: 'Fonte não encontrada.' });
    db.data.sources = db.data.sources.filter(source => source.id !== id);
    const favoriteSet = new Set(db.data.favorites);
    db.data.images = db.data.images.filter(image => {
        if (image.sourceId === id) return favoriteSet.has(image.id);
        return true;
    }).map(image => {
        if (image.sourceId === id) return { ...image, sourceId: null, source: `(Arquivado) ${image.source}` };
        return image;
    });
    await db.write();
...
app.post('/api/favorites', async (req, res) => {
    ...
    if (typeof imageId === 'undefined' || typeof favorite === 'undefined') return res.status(400).json({ message: 'imageId e favorite são obrigatórios.' });
    await db.read();
    const favoriteSet = new Set(db.data.favorites);
    if (favorite) favoriteSet.add(imageId);
    else favoriteSet.delete(imageId);
...
app.get('/api/download', async (req, res) => {
    const { url } = req.query;
    if (!url) return res.status(400).json({ message: 'URL da imagem é obrigatória.' });
    try {
...
// ENDPOINT DE SCRAPE ATUALIZADO COM FILA
app.post('/api/images/scrape', async (req, res) => {
    if (isScraping) {
        return res.status(429).json({ message: 'O servidor está ocupado a processar outro pedido. Por favor, tente novamente em alguns momentos.' });
    }
    ...
    try {
        ...
        if (!url && !isMultiPage) return res.status(400).json({ message: 'URL ou padrão de URL é obrigatório.' });
        ...
        console.log(`POST /api/images/scrape -> A iniciar busca real.`);
        let browser = null;
        try {
            ...
            page.on('request', (req) => {
                if (['stylesheet', 'font', 'script', 'other'].includes(req.resourceType())) req.abort();
                else req.continue();
            });
            ...
            if (finalImageUrls.length === 0) return res.status(404).json({ message: 'Nenhuma imagem de alta qualidade encontrada.' });
            ...
            const newImages = finalImageUrls.map((imageUrl, i) => ({
                id: `scrape_${Date.now()}_${i}`, src: imageUrl, alt: `Imagem de ${newSource.name}`,
                source: newSource.name, author: 'WebScraper', sourceId: newSource.id
            }));
        ...
        isScraping = false; // Liberta a fila para o próximo pedido
    }
});

// ... (endpoint de sync inalterado)
